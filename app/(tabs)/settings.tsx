...
import { User, LogOut, Shield, HelpCircle, Mail, ChevronRight, Zap, Info, Bell, Globe, Gift, Copy, Twitter, MessageCircle, Coins, TrendingUp, Users, Check, X, Edit3, Sun, Moon } from 'lucide-react-native';
...
export default function SettingsScreen() {
  ...
  const { totalHeroCount, totalEarningRate, referralCode } = useMemo(() => ({
    ...
    referralCode: `HERO${profile?.id?.slice(0, 6).toUpperCase() || 'XXXXXX'}`,
  }), [stackedHeroes, profile?.id]);
      ...
      <View style={{ flex: 1, backgroundColor: overlayColor }}>
        <SafeAreaView style={{ flex: 1 }}>
          <ScrollView style={{ flex: 1, paddingHorizontal: 16, paddingTop: 12 }} showsVerticalScrollIndicator={false}>
            ...
            <ReferralSection referralCode={referralCode} copied={copied} onCopy={handleCopyReferral} theme={theme} />
...
const ProfileCard = ({ profile, user, totalHeroCount, totalEarningRate, theme, onEdit }: any) => (
  <View style={{ backgroundColor: theme.colors.card, borderRadius: 14, padding: 14, marginBottom: 18, borderWidth: 1, borderColor: theme.colors.cardBorder }}>
    <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 14 }}>
      <View style={{ width: 52, height: 52, borderRadius: 14, backgroundColor: 'rgba(251, 191, 36, 0.15)', justifyContent: 'center', alignItems: 'center', marginRight: 12 }}>
        <Zap color={theme.colors.primary} size={28} />
      </View>
      <View style={{ flex: 1 }}>
        <Text style={{ fontSize: 16, fontWeight: '700', color: theme.colors.text, marginBottom: 4 }}>{profile?.username || user?.email?.split('@')[0] || 'Hero'}</Text>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
          <Mail color={theme.colors.textMuted} size={12} />
          <Text style={{ fontSize: 11, color: theme.colors.textSecondary }}>{user?.email}</Text>
        </View>
      </View>
      <TouchableOpacity style={{ flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: 'rgba(251, 191, 36, 0.15)', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8 }} onPress={onEdit}>
        <Edit3 color={theme.colors.primary} size={14} />
        <Text style={{ fontSize: 11, fontWeight: '600', color: theme.colors.primary }}>Edit</Text>
      </TouchableOpacity>
    </View>
    <View style={{ flexDirection: 'row', justifyContent: 'space-around', backgroundColor: theme.colors.surfaceSecondary, borderRadius: 10, padding: 12 }}>
      <StatItem icon={Coins} color={theme.colors.primary} value={(profile?.supercash_balance || 0).toLocaleString()} label="SuperCash" theme={theme} />
      <View style={{ width: 1, backgroundColor: theme.colors.divider }} />
      <StatItem icon={Users} color={theme.colors.success} value={totalHeroCount} label="Heroes" theme={theme} />
      <View style={{ width: 1, backgroundColor: theme.colors.divider }} />
      <StatItem icon={TrendingUp} color={theme.colors.info} value={totalEarningRate} label="SC/hr" theme={theme} />
    </View>
  </View>
);
...
const StatItem = ({ icon: Icon, color, value, label, theme }: any) => (
  <View style={{ alignItems: 'center', gap: 4 }}>
    ...
    <Text style={{ fontSize: 14, fontWeight: '700', color: theme.colors.text }}>{value}</Text>
...
const AppearanceSection = ({ isDark, toggleTheme, theme }: any) => (
  <View style={{ marginBottom: 18 }}>
    <Text style={{ fontSize: 14, fontWeight: '700', color: theme.colors.text, marginBottom: 10 }}>Appearance</Text>
    <View style={{ backgroundColor: theme.colors.card, borderRadius: 12, borderWidth: 1, borderColor: theme.colors.cardBorder }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 14 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
          <View style={{ width: 40, height: 40, borderRadius: 10, backgroundColor: isDark ? 'rgba(139, 92, 246, 0.15)' : 'rgba(251, 191, 36, 0.15)', justifyContent: 'center', alignItems: 'center' }}>
            {isDark ? <Moon color={theme.colors.purple} size={20} /> : <Sun color={theme.colors.primary} size={20} />}
          </View>
          <View>
            <Text style={{ fontSize: 14, fontWeight: '600', color: theme.colors.text }}>Dark Mode</Text>
            <Text style={{ fontSize: 11, color: theme.colors.textSecondary }}>{isDark ? 'Currently using dark theme' : 'Currently using light theme'}</Text>
          </View>
        </View>
        <Switch value={isDark} onValueChange={toggleTheme} trackColor={{ false: '#E2E8F0', true: 'rgba(139, 92, 246, 0.5)' }} thumbColor={isDark ? theme.colors.purple : '#FBBF24'} />
      </View>
    </View>
  </View>
);

const ReferralSection = ({ referralCode, copied, onCopy, theme }: any) => (
  <View style={{ marginBottom: 18 }}>
    <Text style={{ fontSize: 14, fontWeight: '700', color: theme.colors.text, marginBottom: 10 }}>Referral Program</Text>
    <View style={{ backgroundColor: theme.colors.card, borderRadius: 12, padding: 14, borderWidth: 1, borderColor: theme.colors.cardBorder }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 14 }}>
        <View style={{ width: 40, height: 40, borderRadius: 10, backgroundColor: 'rgba(251, 191, 36, 0.15)', justifyContent: 'center', alignItems: 'center', marginRight: 12 }}>
          <Gift color={theme.colors.primary} size={20} />
        </View>
        <View style={{ flex: 1 }}>
          <Text style={{ fontSize: 13, fontWeight: '700', color: theme.colors.text }}>Invite Friends & Earn</Text>
          <Text style={{ fontSize: 10, color: theme.colors.textSecondary }}>Get 100 SC for each friend</Text>
        </View>
      </View>
      <View style={{ marginBottom: 12 }}>
        <Text style={{ fontSize: 10, color: theme.colors.textSecondary, marginBottom: 6 }}>Your Referral Code</Text>
        <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: theme.colors.surfaceSecondary, borderRadius: 8, padding: 10, borderWidth: 1, borderColor: `${theme.colors.primary}50`, borderStyle: 'dashed' }}>
          <Text style={{ flex: 1, fontSize: 14, fontWeight: '700', color: theme.colors.primary, letterSpacing: 2 }}>{referralCode}</Text>
          <TouchableOpacity style={[{ padding: 4 }, copied && { backgroundColor: theme.colors.successLight, borderRadius: 4 }]} onPress={onCopy}>
            {copied ? <Check color={theme.colors.success} size={16} /> : <Copy color={theme.colors.primary} size={16} />}
          </TouchableOpacity>
        </View>
        {copied && <Text style={{ fontSize: 10, color: theme.colors.success, marginTop: 6 }}>Copied to clipboard!</Text>}
      </View>
      <View style={{ flexDirection: 'row', gap: 12 }}>
        <View style={{ flex: 1, backgroundColor: theme.colors.surfaceSecondary, borderRadius: 8, padding: 10, alignItems: 'center' }}>
          <Text style={{ fontSize: 16, fontWeight: '700', color: theme.colors.text }}>0</Text>
          <Text style={{ fontSize: 9, color: theme.colors.textSecondary }}>Invites</Text>
        </View>
        <View style={{ flex: 1, backgroundColor: theme.colors.surfaceSecondary, borderRadius: 8, padding: 10, alignItems: 'center' }}>
          <Text style={{ fontSize: 16, fontWeight: '700', color: theme.colors.text }}>0</Text>
          <Text style={{ fontSize: 9, color: theme.colors.textSecondary }}>SC Earned</Text>
        </View>
      </View>
    </View>
  </View>
...
const NotificationsSection = ({ push, email, setPush, setEmail, isDark, theme }: any) => (
  <View style={{ marginBottom: 18 }}>
    <Text style={{ fontSize: 14, fontWeight: '700', color: theme.colors.text, marginBottom: 10 }}>Notifications</Text>
    <View style={{ backgroundColor: theme.colors.card, borderRadius: 12, overflow: 'hidden', borderWidth: 1, borderColor: theme.colors.cardBorder }}>
...
const NotificationRow = ({ icon: Icon, title, desc, color, value, onChange, isDark, theme }: any) => (
  <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 12 }}>
    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
      <View style={{ width: 36, height: 36, borderRadius: 10, backgroundColor: `${color}26`, justifyContent: 'center', alignItems: 'center' }}>
        <Icon color={color} size={18} />
      </View>
      <View>
        <Text style={{ fontSize: 13, fontWeight: '600', color: theme.colors.text }}>{title}</Text>
        <Text style={{ fontSize: 10, color: theme.colors.textSecondary }}>{desc}</Text>
      </View>
    </View>
    <Switch value={value} onValueChange={onChange} trackColor={{ false: isDark ? '#334155' : '#E2E8F0', true: `${color}80` }} thumbColor={value ? color : theme.colors.textMuted} />
  </View>
);

const MenuSection = ({ title, items, theme }: any) => (
  <View style={{ marginBottom: 18 }}>
    <Text style={{ fontSize: 14, fontWeight: '700', color: theme.colors.text, marginBottom: 10 }}>{title}</Text>
    <View style={{ backgroundColor: theme.colors.card, borderRadius: 12, overflow: 'hidden', borderWidth: 1, borderColor: theme.colors.cardBorder }}>
      {items.map((item: any, index: number) => (
        <TouchableOpacity key={index} style={{ flexDirection: 'row', alignItems: 'center', padding: 12, borderBottomWidth: index === items.length - 1 ? 0 : 1, borderBottomColor: theme.colors.divider }} onPress={item.onPress}>
          <View style={{ width: 36, height: 36, borderRadius: 10, backgroundColor: `${item.color}15`, justifyContent: 'center', alignItems: 'center', marginRight: 12 }}>
            <item.icon color={item.color} size={18} />
          </View>
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 13, fontWeight: '600', color: theme.colors.text, marginBottom: 2 }}>{item.title}</Text>
            <Text style={{ fontSize: 10, color: theme.colors.textSecondary }}>{item.description}</Text>
          </View>
...
const SocialSection = ({ theme }: any) => (
  <View style={{ marginBottom: 18 }}>
    <Text style={{ fontSize: 14, fontWeight: '700', color: theme.colors.text, marginBottom: 10 }}>Follow Us</Text>
    <View style={{ flexDirection: 'row', gap: 10 }}>
      {[{ icon: Twitter, color: '#1DA1F2', label: 'Twitter' }, { icon: MessageCircle, color: '#5865F2', label: 'Discord' }, { icon: Globe, color: theme.colors.text, label: 'Website' }].map((item, i) => (
        <TouchableOpacity key={i} style={{ flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, backgroundColor: theme.colors.card, borderRadius: 10, padding: 12, borderWidth: 1, borderColor: theme.colors.cardBorder }}>
