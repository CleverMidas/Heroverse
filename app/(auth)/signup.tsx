...
import { EMAIL_REGEX, USERNAME_REGEX } from '@/lib/validation';
...
interface FieldValidation { isValid: boolean; isChecking: boolean; error: string | null; }
...
export default function SignupScreen() {
  ...
  const checkUsernameExists = useCallback(async (value: string) => {
    ...
    try {
      ...
      if (error) { setUsernameValidation({ isValid: true, isChecking: false, error: null }); return; }
      if (data === true) setUsernameValidation({ isValid: false, isChecking: false, error: 'Username already taken' });
      else setUsernameValidation({ isValid: true, isChecking: false, error: null });
    } catch { setUsernameValidation({ isValid: true, isChecking: false, error: null }); }
  }, []);
  ...
  const checkEmailExists = useCallback(async (value: string) => {
    ...
    try {
      ...
      if (error) { setEmailValidation({ isValid: true, isChecking: false, error: null }); return; }
      if (data === true) setEmailValidation({ isValid: false, isChecking: false, error: 'Email already registered' });
      else setEmailValidation({ isValid: true, isChecking: false, error: null });
    } catch { setEmailValidation({ isValid: true, isChecking: false, error: null }); }
  }, []);

  useEffect(() => {
    if (!username) { setUsernameValidation({ isValid: false, isChecking: false, error: null }); return; }
    if (!USERNAME_REGEX.test(username)) {
      setUsernameValidation({ isValid: false, isChecking: false, error: username.length < 3 ? 'Username must be at least 3 characters' : username.length > 20 ? 'Username must be at most 20 characters' : 'Username can only contain letters, numbers, and underscores' });
      return;
    }
  ...
  useEffect(() => {
    if (!email) { setEmailValidation({ isValid: false, isChecking: false, error: null }); return; }
    if (!EMAIL_REGEX.test(email)) { setEmailValidation({ isValid: false, isChecking: false, error: 'Please enter a valid email address' }); return; }
  ...
  useEffect(() => {
    if (!password) { setPasswordValidation({ isValid: false, isChecking: false, error: null }); return; }
    if (password.length < 6) setPasswordValidation({ isValid: false, isChecking: false, error: 'Password must be at least 6 characters' });
    else setPasswordValidation({ isValid: true, isChecking: false, error: null });
  }, [password]);

  useEffect(() => {
    if (!confirmPassword) { setConfirmPasswordValidation({ isValid: false, isChecking: false, error: null }); return; }
    if (confirmPassword !== password) setConfirmPasswordValidation({ isValid: false, isChecking: false, error: 'Passwords do not match' });
    else if (password.length >= 6) setConfirmPasswordValidation({ isValid: true, isChecking: false, error: null });
    else setConfirmPasswordValidation({ isValid: false, isChecking: false, error: null });
  ...
  const handleSignup = async () => {
    ...
    setLoading(true);
    const { error: signUpError, data: signUpData } = await supabase.auth.signUp({ email, password, options: { data: { full_name: username } } });
    if (signUpError) {
      if (signUpError.message.includes('already registered') || signUpError.message.includes('already exists')) { setError('This email is already registered. Please sign in instead.'); setEmailValidation({ isValid: false, isChecking: false, error: 'Email already registered' }); }
      else if (signUpError.message.includes('valid email')) { setError('Please enter a valid email address.'); setEmailValidation({ isValid: false, isChecking: false, error: 'Invalid email address' }); }
      else setError(signUpError.message);
      setLoading(false); return;
    }
    if (!signUpData.user) { setError('This email is already registered. Please sign in instead.'); setEmailValidation({ isValid: false, isChecking: false, error: 'Email already registered' }); setLoading(false); return; }
    const { error: updateError } = await supabase.from('profiles').update({ username }).eq('id', signUpData.user.id);
    if (updateError) {
      if (updateError.message.includes('unique') || updateError.message.includes('duplicate') || updateError.code === '23505') { setError('This username is already taken. Please choose another.'); setUsernameValidation({ isValid: false, isChecking: false, error: 'Username already taken' }); setLoading(false); return; }
  ...
  const renderFieldStatus = (validation: FieldValidation, hasValue: boolean) => {
    ...
    if (validation.isChecking) return <ActivityIndicator size="small" color="rgba(255, 255, 255, 0.7)" style={styles.fieldStatus} />;
    if (validation.error) return <AlertCircle color="#EF4444" size={18} style={styles.fieldStatus} />;
    if (validation.isValid) return <CheckCircle color="#22C55E" size={18} style={styles.fieldStatus} />;
    return null;
  };
      ...
      <SafeAreaView style={styles.safeArea}>
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.keyboardView}>
          <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
            <View style={styles.container}><View style={styles.content}><View style={styles.header}><Text style={styles.title}>Create Account</Text></View>{error && <View style={styles.errorContainer}><AlertCircle color="#DC2626" size={20} /><Text style={styles.errorText}>{error}</Text></View>}<View style={styles.formContainer}><View><View style={getInputStyle(usernameValidation, !!username)}><User size={20} color="rgba(255, 255, 255, 0.7)" style={styles.inputIcon} /><TextInput style={styles.input} placeholder="Username" placeholderTextColor="rgba(255, 255, 255, 0.5)" value={username} onChangeText={setUsername} autoCapitalize="none" autoComplete="username" />{renderFieldStatus(usernameValidation, !!username)}</View>{usernameValidation.error && <Text style={styles.fieldError}>{usernameValidation.error}</Text>}</View><View><View style={getInputStyle(emailValidation, !!email)}><Mail size={20} color="rgba(255, 255, 255, 0.7)" style={styles.inputIcon} /><TextInput style={styles.input} placeholder="Email" placeholderTextColor="rgba(255, 255, 255, 0.5)" value={email} onChangeText={setEmail} keyboardType="email-address" autoCapitalize="none" autoComplete="email" />{renderFieldStatus(emailValidation, !!email)}</View>{emailValidation.error && <Text style={styles.fieldError}>{emailValidation.error}</Text>}</View><View><View style={getInputStyle(passwordValidation, !!password)}><Lock size={20} color="rgba(255, 255, 255, 0.7)" style={styles.inputIcon} /><TextInput style={[styles.input, styles.inputWithButton]} placeholder="Password" placeholderTextColor="rgba(255, 255, 255, 0.5)" value={password} onChangeText={setPassword} secureTextEntry={!showPassword} autoCapitalize="none" autoComplete="password" />{renderFieldStatus(passwordValidation, !!password)}<Pressable style={styles.eyeButton} onPress={() => setShowPassword(!showPassword)}>{showPassword ? <EyeOff size={20} color="rgba(255, 255, 255, 0.7)" /> : <Eye size={20} color="rgba(255, 255, 255, 0.7)" />}</Pressable></View>{passwordValidation.error && <Text style={styles.fieldError}>{passwordValidation.error}</Text>}</View><View><View style={getInputStyle(confirmPasswordValidation, !!confirmPassword)}><Lock size={20} color="rgba(255, 255, 255, 0.7)" style={styles.inputIcon} /><TextInput style={[styles.input, styles.inputWithButton]} placeholder="Confirm Password" placeholderTextColor="rgba(255, 255, 255, 0.5)" value={confirmPassword} onChangeText={setConfirmPassword} secureTextEntry={!showConfirmPassword} autoCapitalize="none" autoComplete="password" />{renderFieldStatus(confirmPasswordValidation, !!confirmPassword)}<Pressable style={styles.eyeButton} onPress={() => setShowConfirmPassword(!showConfirmPassword)}>{showConfirmPassword ? <EyeOff size={20} color="rgba(255, 255, 255, 0.7)" /> : <Eye size={20} color="rgba(255, 255, 255, 0.7)" />}</Pressable></View>{confirmPasswordValidation.error && <Text style={styles.fieldError}>{confirmPasswordValidation.error}</Text>}</View><TouchableOpacity style={[styles.authButton, (!isFormValid || loading) && styles.authButtonDisabled]} onPress={handleSignup} disabled={loading || !isFormValid} activeOpacity={0.8}><LinearGradient colors={loading ? ['#94A3B8', '#64748B'] : isFormValid ? ['#22C55E', '#16A34A'] : ['#64748B', '#475569']} style={styles.authGradient}>{loading ? <ActivityIndicator color="#FFFFFF" /> : <Text style={styles.authButtonText}>Sign Up</Text>}</LinearGradient></TouchableOpacity><TouchableOpacity style={styles.toggleButton} onPress={() => router.push('/(auth)/login')} disabled={loading} activeOpacity={0.7}><Text style={styles.toggleText}>Already have an account? Sign In</Text></TouchableOpacity><View style={styles.divider}><View style={styles.dividerLine} /><Text style={styles.dividerText}>OR</Text><View style={styles.dividerLine} /></View><TouchableOpacity style={styles.googleButton} onPress={handleGoogleSignup} disabled={loading} activeOpacity={0.8}><View style={[styles.googleButtonContent, loading && styles.buttonDisabled]}>{loading ? <ActivityIndicator color="#FFFFFF" /> : <><View style={styles.googleIcon}><Text style={styles.googleIconText}>G</Text></View><Text style={styles.googleButtonText}>Continue with Google</Text></>}</View></TouchableOpacity></View><Text style={styles.terms}>By continuing, you agree to our Terms of Service and Privacy Policy</Text></View></View>
          </ScrollView>
