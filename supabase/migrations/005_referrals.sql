...
BEGIN
  ...
  SELECT id, referred_by, referral_bonus_claimed, supercash_balance INTO current_user_record FROM profiles WHERE id = current_user_id;
  ...
  SELECT id, username, supercash_balance INTO referrer_record FROM profiles WHERE UPPER(referral_code) = UPPER(code);
  ...
  UPDATE profiles SET referred_by = referrer_record.id, updated_at = now() WHERE id = current_user_id;
  new_referrer_balance := referrer_record.supercash_balance + referral_bonus;
  UPDATE profiles SET supercash_balance = new_referrer_balance, updated_at = now() WHERE id = referrer_record.id;
...
BEGIN
  ...
  IF referrer_id IS NOT NULL THEN SELECT referral_code INTO used_referral_code FROM profiles WHERE id = referrer_id; END IF;
  SELECT COUNT(*) INTO invite_count FROM profiles WHERE referred_by = current_user_id;
  SELECT COALESCE(SUM(amount), 0) INTO total_earned FROM transactions WHERE user_id = current_user_id AND type = 'referral_bonus';
